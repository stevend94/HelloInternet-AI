...
def download_file_with_progress(url_base, sub_dir, model_name, file_name):
    ...
    with open(os.path.join(sub_dir, file_name).replace('\\', "/"), 'wb') as f:
...
def download_gpt2(model_dir='models', model_name='124M'):
    ...
    sub_dir = os.path.join(model_dir, model_name).replace('\\', "/")
...
def finetune(sess,
    ...
    checkpoint_path = os.path.join(checkpoint_dir, run_name).replace('\\', "/")
    ...
    for file in ['hparams.json', 'encoder.json', 'vocab.bpe']:
        try:
            shutil.copyfile(os.path.join(model_dir, model_name, file).replace('\\', "/"),
                            os.path.join(checkpoint_path, file).replace('\\', "/"))
    ...
    with open(os.path.join(checkpoint_path, 'hparams.json').replace('\\', "/")) as f:
    ...
    counter = 1
    counter_path = os.path.join(checkpoint_path, 'counter').replace('\\', "/")
    ...
    def save():
        ...
        print(
            ...
            os.path.join(checkpoint_path,
                         'model-{}').format(counter-1).replace('\\', "/"))
        saver.save(
            sess,
            os.path.join(checkpoint_path, 'model').replace('\\', "/"),
    ...
    def generate_samples():
        ...
        while index < sample_num:
            ...
            for i in range(min(sample_num - index, batch_size)):
                ...
                index += 1
        print(text)
        maketree(os.path.join(SAMPLE_DIR, run_name).replace('\\', "/"))
        with open(
                os.path.join(SAMPLE_DIR, run_name,
                             'samples-{}').format(counter).replace('\\', "/"), 'w') as fp:
            fp.write('\n'.join(all_text))
    ...
    if overwrite and restore_from == 'latest':
        for file in files:
            if file.startswith('model') or file.startswith('events'):
                os.remove(os.path.join(checkpoint_path, file).replace('\\', "/"))
        save()
...
def load_gpt2(sess,
    ...
    if model_name:
        checkpoint_path = os.path.join(model_dir, model_name).replace('\\', "/")
    else:
        checkpoint_path = os.path.join(checkpoint_dir, run_name).replace('\\', "/")
    ...
    if checkpoint=='latest':
        ...
        ckpt = os.path.join(checkpoint_path,checkpoint).replace('\\', "/")
...
def copy_checkpoint_to_gdrive(run_name='run1', copy_folder=False):
    ...
    checkpoint_folder = os.path.join('checkpoint', run_name).replace('\\', "/")
...
def is_gpt2_downloaded(model_dir='models', model_name='124M'):
    ...
    for filename in ['checkpoint', 'encoder.json', 'hparams.json',
                     'model.ckpt.data-00000-of-00001', 'model.ckpt.index',
                     'model.ckpt.meta', 'vocab.bpe']:
        if not os.path.isfile(os.path.join(model_dir, model_name, filename).replace('\\', "/")):
            return False
    return True
...
def cmd_generate(nfiles, nsamples, folder,
    ...
    for _ in trange(nfiles):
        gen_file = os.path.join(folder,
                    'gpt2_gentext_{:%Y%m%d_%H%M%S}.txt'.format(datetime.utcnow())).replace('\\', "/")
